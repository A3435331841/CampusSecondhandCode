...
- 后台用户分页与状态更新
- 管理员登录
- 订单创建与闭环（取消、确认收货、库存回补）
- Redis Lua 预扣库存防超卖

**B 范围功能（已全部完成）：**

- 微信小程序登录（真实接入：`uni.login` → `code2Session` → Redis Token）
- 学号实名认证（模拟但可替换：本地 `biz_student_roster` 名单校验）
- 商品收藏与评论
- WebSocket 即时通讯（会话持久化、历史消息查询）
- 腾讯地图面交点选择（真实接入：地图搜索、逆地址解析、校园地点兜底）
- 评价与信用闭环（双方评价、信用分更新）
- 分类管理（管理端 CRUD、用户端 API 驱动）
- 管理端运营统计看板（ECharts 图表）

第三方接入区分：
- 微信登录和腾讯地图为真实第三方接入，需在对应平台申请密钥
- 学号实名使用本地学生名册表，后续可替换为学校官方接口
- 图片上传使用本地文件存储
...
| 鉴权     | Token + Redis                      | 拦截器统一校验             | 已实现 Token 登录态管理              |
| 实时通信 | WebSocket                          | 支撑 IM 聊天               | 已实现会话持久化与历史消息查询       |
| 文件存储 | 本地文件存储                       | 支撑图片上传               | 已实现 `/api/file/upload` 接口     |
...
| 分类     | `biz_category`          | 已实现   | 管理端 CRUD + 用户端 API 驱动 |
| 商品图片 | `biz_product_image`     | 已实现   | 支持多图上传                 |
| 收藏     | `biz_favorite`          | 已实现   | 幂等收藏，后端计数           |
| 评论     | `biz_product_comment`   | 已实现   | 详情页展示                   |
| 聊天消息 | `biz_chat_session`、`biz_chat_message` | 已实现 | WebSocket 实时 + 历史查询 |
| 评价     | `biz_order_review`      | 已实现   | 双方评价，信用分联动         |
...
B 范围功能已全部完成，当前保留的演进项包括：

- 生产环境部署脚本（CI/CD）
- 学校官方认证接口直连（当前使用本地学籍名单）
- 真实线上支付集成
- 机器学习推荐（当前为基于行为的轻量规则推荐）
